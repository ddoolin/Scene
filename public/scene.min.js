window.Scene.HomeController = function() {
    ...
    this.createMap = function(b) {
        ...
        marker = a.createColoredMarker("yellow", [ e, f ]), a.populateMap();
        ...
        c = new google.maps.Marker({
            ...
            shadow: e,
            icon: d
        }), window.Scene.map.setCenter(b), c;
    }, this.createInfoWindow = function(b, c) {
        var d = "<div class='content'>";
        c.image && (d += "<img src='" + c.image + "' class='photo' />"), d += "<a class='first-heading' href='/events/" + c._id + "''>" + c.name + "</a><div class='body-content'>" + "<p class='time'>" + a.formatDate(new Date(c.duration.starttime)) + " @ " + a.formatTime(new Date(c.duration.starttime)) + "</p>", 
        d += window.Scene.user ? "<p class='description logged'>" + c.description + "</p>" + "<a class='attend-event' href='#'>Attend this event</a>" : "<p class='description'>" + c.description + "</p>", 
        d += "</div></div>", google.maps.event.addListener(b, "click", function() {
            window.Scene.infoWindow.setContent(d), window.Scene.infoWindow.open(window.Scene.map, this), 
            $(".attend-event").on("click", function(b) {
                b.preventDefault(), window.socket.emit("User.attendEvent", {
                    user: window.Scene.user._id,
                    event: c._id
                }), a.addAttendingEvent(c);
            });
        });
    }, this.populateMap = function() {
        window.Scene.events, _.each(window.Scene.events, function(b) {
            marker = a.createMarker([ b.location.latitude, b.location.longitude ]), a.createInfoWindow(marker, b);
        });
        ...
        console.log("Here");
    }, this.addAttendingEvent = function(a) {
        var b = "<div class='event'><div class='event-name'><a href='/events/" + a._id + "'>" + a.name + "</a></div>" + "<div class='event-duration'>" + moment(a.duration.starttime).format("MM/DD h:mma") + " ~ " + moment(a.duration.endtime).format("MM/DD h:mma") + "</div>" + "<div class='event-location'>" + a.address + "</div></div>";
        $("#sidebar .events").prepend(b);
        ...
        if ($(".spot-text").val()) {
            ...
            a.geocode({
                ...
                if (a) {
                    ...
                    window.socket.emit("User.addSpot", b);
                }
            });
        }
    }, this.renderSpot = function(b) {
        var b = b.spot, c = "<div class='spot'><a class='spot-address' href='#' data-lat='" + b.latitude + "' data-lng='" + b.longitude + "'>" + b.address + "</a></div>";
        $("#sidebar .spots").prepend(c), el = $("#sidebar .spot").get(0), console.log($(el)), 
        $(el).click(function(b) {
            b.preventDefault(), a.centerSpot($(el).data("lat"), $(el).data("lng"));
        });
    }, this.centerSpot = function(a, b) {
        var c = new google.maps.LatLng(a, b);
        window.Scene.map.setCenter(c);
    };
}, function() {
    "use strict";
    window.Scene.EventView = Backbone.View.extend({
        el: ".event",
        initialize: function() {
            _.bindAll(this, "onPhotoCreate", "onPhotoUpdate"), this.listenTo(this.model, "change", this.render), 
            this.photo_views = {}, this.model.get("photos").forEach(this.onPhotoCreate);
        },
        onPhotoCreate: function(a) {
            var b = new window.Scene.PhotoView({
                model: new window.Scene.Photo(a)
            });
            this.$el.append(b.$el), this.photo_views[a._id] = b;
        },
        onPhotoUpdate: function(a) {
            this.photo_views[a._id].model.set("position", a.position);
        },
        registerWSEvents: function(a) {
            a.on("Photo.create", this.onPhotoCreate), a.on("Photo.update", this.onPhotoUpdate);
        },
        render: function() {
    ...
    (function() {
        ...
        a.on("disconnect", function() {}), a.on("Event.create", function(a) {
            marker = c.createColoredMarker("green", [ a.location.latitude, a.location.longitude ]), 
            c.createInfoWindow(marker, a);
        }), a.on("User.addSpot", function(a) {
            c.renderSpot(a);
        })), a.socket.connect(), window.socket = a;
    }(), function() {
        _.each(a("#sidebar .event-duration"), function(b) {
            var c = a(b).get(0), d = moment(a(c).data("start-time")).format("MM/DD, h:mma"), e = moment(a(c).data("end-time")).format("MM/DD, h:mma");
            a(c).text(d + " ~ " + e);
        });
    }(), function() {
        a("#login").popover({
        ...
        }), a("#cancel_registration").click(function(b) {
            b.preventDefault(), a("#registration_modal").modal("hide");
        }), a("#user_submit").click(function(a) {
            a.preventDefault(), c.createUser();
            ...
            a(b.target).siblings(".hour-select").hide();
        }), c.setDefaultTimes(), a(".hour").mousedown(function(b) {
            parentInput = a(b.target).parent().siblings().get(1), a(parentInput).val(a(b.target).text());
        ...
        }), a(".spots-container").height(a("#sidebar").height() - 36), a(".attending-tab").click(function() {
            a(".spots-tab").removeClass("active"), a(".attending-tab").addClass("active"), a(".spots").hide(), 
            a(".events").show();
        }), a(".spots-tab").click(function() {
            a(".attending-tab").removeClass("active"), a(".spots-tab").addClass("active"), a(".events").hide(), 
            a(".spots").show();
        }), a("#add_spot").click(function(a) {
            a.preventDefault(), c.addSpot();
        }), a(".spot-address").click(function(b) {
            b.preventDefault(), c.centerSpot(a(b.target).data("lat"), a(b.target).data("lng"));
        });
    }();
}(jQuery), function() {
    "use strict";
    window.Scene || (window.Scene = {}), window.Scene.PhotoDetailView = Backbone.View.extend({
        el: "#photo_detail_modal",
        initialize: function() {},
        show: function(a) {
            this.model = a, this.render(), this.$el.modal("show");
        },
        hide: function() {
            this.$el.modal("hide");
        },
        render: function() {
            this.$el.find(".photo_img").attr("src", this.model.get("image")), this.$el.find(".comment_list");
        }
    });
}(jQuery), function(a) {
    "use strict";
    window.Scene || (window.Scene = {}), window.Scene.PhotoView = Backbone.View.extend({
        ...
        initialize: function() {
            _.bindAll(this, "onClick", "onMoved"), this.listenTo(this.model, "change", this.render), 
            this.$el.click(this.onClick), this.$el.draggable({
                cursor: "crosshair"
            }).on("drag", this.onMoved).css({
                position: "absolute"
        ...
        onMoved: function() {
            this.model.get("size"), this.model.set("position", {
                x: 100 * (this.$el.position().left / a(".event").width()),
                y: 100 * (this.$el.position().top / a(".event").height())
            }), window.Scene.socket.emit("Photo.update", this.model.toJSON());
        },
        onClick: function() {
            window.Scene.photoDetailView && window.Scene.photoDetailView.show(this.model);
        },
        render: function() {
